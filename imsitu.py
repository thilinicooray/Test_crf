...
class imSituVerbRoleNounEncoder:
    ...
    for img in dataset:
      for region in img['regions']:
        ...
        if v not in self.v_id:
          ...
          self.v_r[_id]  = []
        vid = self.v_id[v]
        #frame = {(r,n) for (r,n) in frame_.items() if n != 'predicate' }
        for (r,n) in frame_.items():
          if r != "predicate":
  ...
  def to_tensor(self, situations, use_role = True, use_verb = True):
    ...
    for situation in situations:
      #print('curr situation', situation)
      final_list = []
      _rv_list = self.encode(situation)
      #print('encoded regions', len(_rv_list))
      for _rv in _rv_list :
        rv = []
        verb = _rv["verb"]
        items = []
        ...
        #print(rv, 'cat', torch.cat(rv))
        final_list.append(torch.LongTensor(items))
    #return torch.cat(rv)
    #print('final' , len(final_list))
    if len(final_list) > 0:
      b = torch.stack(final_list)
    else:
      #print('no items')
      b = torch.LongTensor(len(situations[0]['regions']), 39).fill_(-1)
    #print('b' , b.size())
    return b
...
class imSituVerbRoleLocalNounEncoder(imSituVerbRoleNounEncoder):
  ...
  def encode(self, situation):
    ...
    for region in situation['regions']:
      ...
      if frame_["predicate"] not in self.v_id:
        continue
      v = self.v_id[frame_["predicate"]]
      rv = {"verb": v, "frames": []}
...
class imSituSimpleImageFolder(data.Dataset):
 # partially borrowed from ImageFolder dataset, but eliminating the assumption about labels
   ...
   def __getitem__(self, index):
        ...
        return img,im_info, torch.LongTensor([index])
...
class imSituSituation(data.Dataset):
   def __init__(self, root, annotation_file, encoder, transform=None):
        ...
        self.ids = self.get_keys(self.imsitu)
   ...
   def __getitem__(self, index):
        ...
        return (torch.LongTensor([index]), img, im_info, target)
