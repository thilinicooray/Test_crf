...
class visual_genome(data.Dataset):
    def __init__(self, set_option, image_set, annotation_folder='top_150_50'):
        ...
        ann_file_name = {'vg_normal_train': 'train.json', 
                           ...
                           'vg_srl_train': 'train_srl.json',
                           'vg_srl_val': 'val_srl.json',
                           'vg_srl_test': 'test_srl.json'
                         }

        ann_file_path = osp.join(annotation_dir, ann_file_name[self.name])
        self.annotations = json.load(open(ann_file_path))
        self.max_size = 35 # including the <end> token excluding <start> token
    ...
    def __getitem__(self, index):
        ...
        #     print(reg['phrase'], self.max_size, reg['region_id'])
        #     val = self.max_size-len(reg['phrase'])
        #     print((0,self.max_size-len(reg['phrase'])))
        #     val_updated = (0,val)
        #     a = np.pad(reg['phrase'],(0,self.max_size-len(reg['phrase'])),'constant',constant_values=self.voc_sign['end'])
        #     list.append(a)
        #
        # print('done')
        ...
        return img, im_info, gt_boxes_region
    ...
    def tokenize_annotations(self, max_size):
        ...
        for im in self.annotations:
            ...
            for region in list(im['regions']):
                complete_sent = (['<start>'] + region['phrase'] + ['<end>'])
                if len(complete_sent) >  max_size:
                    print('big sent ', complete_sent, max_size)
                    diff = len(complete_sent) - max_size
                    region['phrase'] = region['phrase'][: (- diff)]
