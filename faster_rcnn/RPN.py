...
class RPN(nn.Module):
    ...
    @property
    def loss(self):
        return self.cross_entropy_region + 1. * self.loss_box_region

    def forward(self, im_data, im_info, gt_regions=None, dontcare_areas=None):
        ...
        # object proposal score
        rpn_cls_score_region = self.score_conv_region(rpn_conv1_region)
        ...
        rpn_cls_prob_region_reshape = self.reshape_layer(rpn_cls_prob_region, self.anchor_num_region*2)
        # rpn boxes
        rpn_bbox_pred_region = self.bbox_conv_region(rpn_conv1_region)
        ...
        if self.training:
            ...
            if DEBUG:
                print 'rpn_data', rpn_data_region
                print 'rpn_cls_score_reshape', rpn_cls_score_region
