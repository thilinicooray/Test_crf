...
def proposal_target_layer(region_rois,
    ...
    if is_training:
        ...
        assert np.all(all_rois_region[:, 0] == 0), \
            'Only single item batches are supported'

        region_labels, region_rois, bbox_targets_region, bbox_inside_weights_region, \
                = _sample_rois(all_rois_region, \
                    gt_regions, 1, n_classes_obj, voc_eos, is_training)
        ...
        region_rois  = _setup_connection(region_rois)
        region_labels, \
             bbox_targets_region, bbox_inside_weights_region, bbox_outside_weights_region= [None] * 4
    ...
    if DEBUG:
        ...
        count = 1
        fg_num = (region_labels > 0).sum()
        bg_num = (region_labels == 0).sum()
        ...
        count_rel = 1
        fg_num_rel = (region_labels > 0).sum()
        bg_num_rel = (region_labels == 0).sum()
    ...
    assert region_rois.shape[1] == 5
    assert region_rois.shape[1] == 5

    return region_labels, region_rois, \
...
def _sample_rois(region_rois, gt_regions, num_images, num_classes, voc_eos, is_training):
...
### prepare region targets
    region_labels, region_rois, bbox_targets_region, bbox_inside_weight_region = \
                    _sample_regions(region_rois, gt_regions, num_images, voc_eos)
...
### prepare connection matrix
    ...
    return region_labels, region_rois, bbox_targets_region, bbox_inside_weight_region


def _setup_connection(region_rois):
    """Generate a random sample of RoIs comprising foreground and background
    examples.
    """
    # overlaps: (rois x gt_boxes)
...
def _sample_regions(region_rois, gt_regions, num_images, voc_eos):
    """Generate a random sample of RoIs comprising foreground and background
    examples.
    """
    ...
    return labels, rois, bbox_targets, bbox_inside_weight
