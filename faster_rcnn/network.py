...
def get_optimizer_dvsrl(lr, mode, optimizer_type, args,
                        frcnn_features, crf_features, weight_decay):
    """ To get the optimizer
    mode 0: fixed frcnn, train crf
    """
    if mode == 0:
        set_trainable_param(crf_features, True)
        if optimizer_type == 0:
            optimizer = torch.optim.SGD([
                {'params': crf_features}
            ], lr=lr, momentum=args.momentum, weight_decay=0.0005, nesterov=args.nesterov)
